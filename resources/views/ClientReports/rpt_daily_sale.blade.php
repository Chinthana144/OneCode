...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('rptDailySaleSearch.search') }}" method="get">
                <div class="row">
                    <div class="col-md-5">
            ...
            <table class="table">
                ...
                @foreach ($sale as $sal)
                    <tr>
                        ...
                        <td>{{ Str::substr($sal->purchaseDateTime, 0, 10) }}</td>
                        <td>{{ $sal->customer->fullname }}</td>
                        <td>{{ $sal->customer->username }}</td>
                        <td>{{ $sal->package->name }}</td>
                        <td>{{ $sal->package->duration }} days</td>
                        <td>{{ $sal->price }}</td>
                    </tr>
                @endforeach
            </table>
